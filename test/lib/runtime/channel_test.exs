defmodule Ask.Runtime.ChannelTest do
  use Ask.ConnCase
  use Ask.DummySteps
  use Timex

  alias Ask.{Respondent, RespondentGroupChannel, Schedule, TestChannel, BrokerStub, Survey}
  alias Ask.Runtime.{VerboiceChannel, NuntiumChannel, Flow, ReplyHelper, SurveyLogger, Broker, ChannelStatusServer}

  require Ask.Runtime.ReplyHelper

  @survey %{schedule: Ask.Schedule.always()}

  test "ignore respondent replies from channels other than the current one", %{conn: conn} do
    ...
    conn = VerboiceChannel.callback(conn, %{"respondent" => respondent.id, "Digits" => "1"}, Broker)
    conn = recycle(conn)
    conn = NuntiumChannel.callback(conn, %{"channel" => sms_channel.name, "from" => "sms://#{respondent.sanitized_phone_number}", "body" => "this message should be ignored"}, Broker)
    ...
    Repo.get(Respondent, respondent.id) |> IO.inspect
